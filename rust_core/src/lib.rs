...
/// Helper to convert any Python object to an Interval for comparison
/// If it's already a PyInterval, extracts the inner Interval
/// Otherwise, converts the value to a singleton interval
fn other_to_interval(py: Python<'_>, other: &Bound<'_, PyAny>) -> PyResult<Interval> {
    // Try to extract as PyInterval first
    if let Ok(interval) = other.extract::<PyInterval>() {
        return Ok(interval.inner);
    }
    // Otherwise, convert value to singleton interval
    let value = py_to_value(py, other)?;
    Ok(Interval::from_atomic(BoundType::Closed, value.clone(), value, BoundType::Closed))
}
...
impl PyInterval {
    ...
    fn __lt__(&self, py: Python<'_>, other: &Bound<'_, PyAny>) -> PyResult<bool> {
        let other_interval = other_to_interval(py, other)?;
        if self.inner.is_empty() || other_interval.is_empty() {
            return Ok(false);
        }
        ...
        let b_lower = &other_interval.intervals.first().unwrap().lower;
        let b_left = other_interval.intervals.first().unwrap().left;

        Ok(match a_upper.cmp(b_lower) {
    ...
    fn __le__(&self, py: Python<'_>, other: &Bound<'_, PyAny>) -> PyResult<bool> {
        ...
        let a_upper = &self.inner.intervals.last().unwrap().upper;
        let a_right = self.inner.intervals.last().unwrap().right;
        let b_upper = &other_interval.intervals.last().unwrap().upper;
        let b_right = other_interval.intervals.last().unwrap().right;

        Ok(match a_upper.cmp(b_upper) {
            std::cmp::Ordering::Less => true,
            std::cmp::Ordering::Greater => false,
            std::cmp::Ordering::Equal => {
